<template>
    <div class="item" ref="item">
        ...
        <div class="buttons" v-if="this.$route.meta.role == 'user'">
            <button @click="this.$emit('qtEvent', this, false)" class="minus" ref="minus">
                -
            </button>
            <button @click="this.$emit('qtEvent', this, true)" class="plus" ref="plus">
                +
            </button>
        </div>
        <div ref="emptystars" class="rating">
            <img src="https://i.imgur.com/dm2TUZC.png" />
            <div ref="fullstarsdiv" class="rating-stars">
                <img ref="fullstars" src="https://imgur.com/eif22DG.png" />
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    mounted() {
        ...
        var cartQt = this.$store.getters.cart[this.product.id]
            ? this.$store.getters.cart[this.product.id]
            : 0;
        ...
        if (this.$route.meta.role != "user") {
            ...
            this.setButtonColor("plus", this.qt != 0)
            this.setButtonColor("minus", this.qt != this.product.quantity)
        }


    },
    updated() {
        if (this.qt <= 5) this.color();
        this.$refs.fullstars.style.marginRight =
            "-" + (380 - 76 * this.rate) + "px";
    },
    methods: {
        setButtonColor(button, mode) {
            if (mode) {
                ...
                } else this.$refs[button].style.backgroundColor = "green";
            } else this.$refs[button].style.backgroundColor = "grey";
        },
        color() {
            switch (this.qt) {
                case 5:
        ...
        setButtonVisibility(button, mode) {
            if (mode) this.$refs[button].style.visibility = "visible";
            else this.$refs[button].style.backgroundColor = "grey";
        },
    },
};
